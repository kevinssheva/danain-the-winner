...
export default function Sidebar(props: SidebarProps) {
  ...
  return (
    <div className="fixed md:static z-50 py-8 md:py-0 px-[5%] overflow-hidden">
      ...
      <aside
        id="default-sidebar"
        className={`fixed md:absolute top-0 left-0 z-10 w-64 h-screen transition-transform ${open ? "translate-x-0" : "-translate-x-full"
          } md:translate-x-0`}
        aria-label="Sidebar"
      >
            ...
            {sidebarItems.slice(0, -2).map((item) => (
              <li key={item.type}>
                <a
                  ...
                  className={`flex items-center py-3 px-4 rounded-xl ${item.type === "home"
                    ? pathName === "/dashboard"
                      ? "bg-gray-700"
                      : "bg-none"
            ...
            {sidebarItems.slice(-2).map((item) => (
              <li key={item.type} className="cursor-pointer">
                <a
                  ...
                  className={`flex items-center py-3 px-4 rounded-xl ${pathName === `/dashboard/${item.type.toLowerCase()}`
                    ? "bg-gray-700"
                    : "bg-none"
                    } hover:bg-gray-700 group`}
                >
